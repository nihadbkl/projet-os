...
# (we dont use a single cell lock here because the movement passes through MovementManager)
robot_colors = ["red","blue","green","orange","purple","cyan","magenta","yellow","pink","brown","gray","lime"]

# ----------------------------
#the user interface and update mechanism are threads
# ----------------------------
root = tk.Tk()
...
# A queue to securely update the user interface from threads
ui_queue = Queue()
...
# paths are drawn for each robot (IDs of canvas items)
path_items = {}  # rid -> [item,...]
follow_rect = None
...
def _do_update_cell(x, y, robot_id):
    ...
    if not path:
        path_items[rid] = []
        return
    # convert each cell center to coordinates
    coords = []
    for (x,y) in path:
...
# ----------------------------
# long distance BFS better than A algorithm
# ----------------------------
def astar(start, goal):
    ...
    while open_set:
        ...
        if node == goal:
            return path[1:]  # اexclude the current cell
        if node in closed:
            continue
        closed.add(node)
        x,y = node
...
# ----------------------------
# smart collision avoidance (as before)
# ----------------------------
...
# ----------------------------
# center movement manager grants pulls step by step
# ----------------------------
class MovementManager(threading.Thread):
    def init(self):
        super().init(daemon=True)
        self.lock = threading.Lock()
        self.counter = 0  # to calculate request time 
    ...
    def request_move(self, robot_id, from_pos, to_pos, prio):
        ...
        with self.lock:
            self.counter += 1
            key = (prio, self.counter)  # اpriority then time request 
        ...
        while self.running:
            ...
            with self.lock:
                if not self.requests:
                    ...
                    # during the heap iteration, copy a list to process without changing the heap
                    temp = []
                    while self.requests:
                    ...
                    for req in temp:
                        ...
                        # if the requested filed is empty or owned by the same bot (edge)
                        if grid[tx][ty] == 0:
                            # get permission:
                            grid[tx][ty] = rid  # temporarily place the bot in the same bot
                            # we initially leave the old field for its offspring after telling the bot
                            ev.set()
                            granted_any = True
                        else:
                            # cannot be granted now_we retry with inreased time priority (sooner retry later)
                            remaining.append(req)
                    # ضع الطلبات الباقية مجدداً في self.requests مع تحديث counter
                    for req in remaining:
                        heapq.heappush(self.requests, req)
            # after salting permission,wait a little to allow tthe bots to perform the step and release the old one
            if not granted_any:
                time.sleep(0.02)
            else:
                time.sleep(0.01)
...
# ----------------------------
# bot behavior(each bot requests permission step by step)
# ----------------------------
...
def find_empty_start():
  ...
  def robot_thread(rid, stop_ev):
    ...
    x, y = sx, sy
    prio = robot_priority.get(rid, rid)  # id priority :default
    ...
    while not stop_ev.is_set():
        ...
        for step in list(path):
            ...
            schedule_update_stats()
            x,y = nx, ny
            # after the movement, a time interval accroding to the speed
            time.sleep(get_speed_ns())
        schedule_clear_path(rid)
        # continue the loop , choose a new target

    # end:release the current cell if it's still occupied
    try:
...
# ----------------------------
# robot management (Start/Stop/Add/Pause/Priority)
# ----------------------------
...
# ----------------------------
# user interface:selection,buttons,follow,control priority sorting speed 
# ----------------------------
...
# stop the motion manager on exit
